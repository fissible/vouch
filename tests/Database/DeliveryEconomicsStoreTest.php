...
it('permits a decoy without creating or charging spend state', function (): void {
    expect(deliveryEconomics()->reserve(deliveryRequest(decoy: true)))
        ->toBe(DeliveryEconomicsDecision::Permitted)
...
it('refuses an SMS country before creating spend state', function (): void {
    expect(deliveryEconomics()->reserve(deliveryRequest(channel: 'sms', country: 'GB')))
        ->toBe(DeliveryEconomicsDecision::Refused)
        ->and(DB::table('auth_delivery_spend')->count())->toBe(0);
});
...
it('rolls back the global reservation when the tenant ceiling refuses', function (): void {
    ...
    expect($economics->reserve(deliveryRequest(10)))->toBe(DeliveryEconomicsDecision::Permitted)
        ->and($economics->reserve(deliveryRequest(10)))->toBe(DeliveryEconomicsDecision::Refused);
...
it('enforces the ceiling with an atomic predicate rather than a PHP increment', function (): void {
    ...
    expect($economics->reserve(deliveryRequest(7)))->toBe(DeliveryEconomicsDecision::Permitted)
        ->and($economics->reserve(deliveryRequest(7)))->toBe(DeliveryEconomicsDecision::Refused)
...
it('keeps tenant absence and tenant names in separate spend buckets', function (): void {
    ...
    expect($economics->reserve(deliveryRequest(10, null)))->toBe(DeliveryEconomicsDecision::Permitted)
        ->and($economics->reserve(deliveryRequest(10, '')))->toBe(DeliveryEconomicsDecision::Permitted);
