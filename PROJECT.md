...
Final measured state: `composer test` — 130 passed, 355 assertions. `composer stan` —
level 9 clean over `src` and `tests`. `composer mutate` — 86.22% full MSI (≥ 80 floor),
96.97% covered MSI (≥ 95 floor).
...
Final gate state, all green: `composer test` (130 passed, 355 assertions),
`composer stan` (level 9 clean over `src` and `tests`), `composer mutate` (86.22% full
MSI / 96.97% covered MSI, both above their committed floors). `src/Kernel` depends on
